...
class Opa:
    ...
    async def _on_record_event(self, profile: Profile, event: Event):
        ...
        if topic == "present_proof":
            ...
            if event.payload["state"] == "presentation_sent":
                self._pres_req_id = {
                    event.payload["presentation_exchange_id"]: "presentation_sent"
                }

            if (
                self._pres_req_id.get(event.payload["presentation_exchange_id"], None)
                == "presentation_sent"
                and event.payload["state"] == "request_received"
            ):
                return

            indy_proof_request = event.payload["presentation_request"]
            ...
            try:
                req_creds = await indy_proof_req_preview2indy_requested_creds(
                    ...
                    holder=self._context.inject(IndyHolder),
                )

            except ValueError as err:
                LOGGER.warning(f"{err}")
                pass  # not a protocol error: prover could still build proof manually
            event.payload["req_creds"] = req_creds
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if action:
                    ...
                    # Handle actions of type "request proof"
                    elif action["type"] == "request proof":
                    ...
                    elif action["type"] == "present proof":
                        LOGGER.info("Present proof")
                        connection_id = event.payload["connection_id"]
                        req_creds["self_attested_attributes"] = action["data"][
                            "self_attested_attr"
                        ][0]
                        req_creds["requested_attributes"] = {}
                        req_creds["requested_predicates"] = {}
                        session = await profile.session(self._context)
                        responder = session.inject(BaseResponder, required=True)
